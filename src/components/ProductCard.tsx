...
export function ProductCard({ product }: ProductCardProps) {
  return (
    <article className="group premium-card relative flex h-full flex-col overflow-hidden rounded-[2rem] p-4 transition duration-300 hover:-translate-y-2 hover:border-cyan-300/40 hover:shadow-cyan-500/10">
      {product.newArrival ? (
        <span className="absolute right-5 top-5 z-10 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.22em] text-cyan-200">
          New
        </span>
      ) : null}
      ...
      <div className="flex flex-1 flex-col px-2 pb-2 pt-5">
        ...
        <div className="mt-auto flex items-end justify-between gap-4 pt-6">
          ...
          <Link
            ...
            className="rounded-full bg-white px-4 py-2 text-sm font-bold text-black transition hover:bg-cyan-200"
          >
            View
          </Link>
        </div>
